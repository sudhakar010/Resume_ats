...
## streamlit app
st.markdown(
    "<h1 style='color: #ff6347;'>Smart ATS</h1>", unsafe_allow_html=True
)
...
if submit:
    if uploaded_file is not None:
        ...
        input_data = input_prompt.format(text=text, jd=jd)
        response=get_gemini_repsonse(input_data)
        response_dict = json.loads(response)
        st.subheader("Analysis Result:")
        st.write(f"JD Match: {response_dict['JD Match']}%")
        st.write(f"Missing Keywords: {', '.join(response_dict['MissingKeywords'])}")
